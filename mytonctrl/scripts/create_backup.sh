...
# Get arguments
while getopts d:m:t:b:k:u: flag
do
	case "${flag}" in
		d) dest=${OPTARG};;
    m) mtc_dir=${OPTARG};;
...
cp $db_dir/config.json ${tmp_dir}/db
cp -r $db_dir/keyring ${tmp_dir}/db || exit 3  # make sure keyring is copied
if [ -f "$db_dir/collators-list.json" ]; then
	cp "$db_dir/collators-list.json" ${tmp_dir}/db
else
	echo '{}' > ${tmp_dir}/db/collators-list.json
fi
